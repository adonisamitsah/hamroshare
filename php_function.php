...
<head>
    ...
    <style>
        body { background-color: #0f1113; color: #e2e8f0; font-family: "Inter", sans-serif; margin: 0; overflow-x: hidden; }
        ...
        #menu.collapsed .status-card { display: none; }
        #menu.collapsed .pure-menu-heading { justify-content: center; padding: 24px 0 !important; }

        #main { flex-grow: 1; background: #0d1117; width: 100%; transition: all 0.3s ease; }
...
/**
 * Master Dashboard Metrics Engine
 * Fetches all active tokens, calculates wealth, EDIS errors, and sorts urgent renewals.
 */
function getDashboardMetrics($db) {
    // 1. Fetch Reports & Tokens from constant cache table (Excluding renewal_report)
    ...
    // 3. Process Renewals DYNAMICALLY from live user payloads to prevent stale metrics
    $urgentRenewals = [];
    $totalRenewalIssues = 0;
    $date_today = new DateTime('today');
    
    // We fetch only active records that contain operational logs
    ...
    if ($results) {
        while ($row = $results->fetchArray(SQLITE3_ASSOC)) {
            ...
            if ($jsonStart !== false) {
                ...
                // Track indicators across both account mechanisms
                $checks = [
                ...
                foreach ($checks as $type => $dateStr) {
                    ...
                    // Clean string variant to ignore appended timestamps if present
                    ...
                    try {
                        ...
                        // Capture accounts that are within a 60-day window or overdue
                        if ($days__ < 60) {
                            $totalRenewalIssues++;
                            $urgentRenewals[] = [
                                'name' => $displayName,
                        ...
                        // Suppress formatting glitches gracefully
                        continue; 
                    }
                }
            }
    ...
    if (!empty($edisData['payload']['companyPayload'])) {
    ...
    // 5. Structure payload back to UI layers perfectly intact
    return [
        'tokens' => [
            'allshares'  => $allsharesData['token'] ?? null,
            'edis'       => $edisData['token'] ?? null,
            'renewal'    => 'LIVE', // Labeled as Live instead of returning a static token
        ...
        'scanner_logs' => $scannerData['payload']['processing_report'] ?? []
    ];
}
...
// function reconstructCompanyPayloadFromTable($db) {
//     $payload = [];
//     $stmt = $db->query("SELECT * FROM ipo_results ORDER BY scrip ASC");
    
//     while ($row = $stmt->fetchArray(SQLITE3_ASSOC)) {
//         $scrip = $row['scrip'];
//         $status = $row['statusName'];
        
//         if (!isset($payload[$scrip])) {
//             $payload[$scrip] = [
//                 'meta' => ['scrip' => $scrip, 'companyName' => $row['companyName']],
//                 'statuses' => []
//             ];
//         }
        
//         if (!isset($payload[$scrip]['statuses'][$status])) {
//             $payload[$scrip]['statuses'][$status] = [];
//         }
        
//         $payload[$scrip]['statuses'][$status][] = [
//             'name' => 'Unknown', // The table didn't have 'name' column in your schema
//             'dmat' => $row['dmat_num'],
//             'kitta' => $row['receivedKitta'],
//             'old' => 'N/A'
//         ];
//     }
//     return $payload;
...
function alertMessage($type, $message, $includeTimestamp = false) {
    ...
    $map = [
        ...
        'success' => "✅", 'suc' => "✅", 'succ' => "✅", 'ok' => "✅", 'done' => "✅",
        'error'   => "❌", 'err' => "❌", 'fail' => "❌", 'failed' => "❌", 'fatal' => "❌",
        'warning' => "⚠️", 'warn' => "⚠️", 'alert' => "⚠️",
        'notice'  => "ℹ️", 'info' => "ℹ️", 'note' => "ℹ️", 'log' => "ℹ️",
        'denied'  => "🚨", 'danger' => "🚨", 'critical' => "🚨", 'block' => "🚨",
        
        // IPO Result Statuses
        'allotted'     => "🎯", 'alloted' => "🎯", 'allot' => "🎯", 'win' => "🎯",
        'notallotted'  => "⭕", 'notalloted' => "⭕", 'none' => "⭕", 'miss' => "⭕", 'notallot' => "⭕",
        'rejected'     => "🚫", 'reject' => "🚫", 'cancel' => "🚫",
        'verified'     => "☑️", 'veri' => "☑️", 'verify' => "☑️",
        'unverified'   => "⏳", 'unveri' => "⏳", 'pending' => "⏳",
        'neverchecked' => "❓", 'never' => "❓", 'unknown' => "❓",

        // HamroShare Specific
        'applied' => "🟣", 'apply' => "🟣",
        'skipped' => "⏭️", 'skip' => "⏭️", 'bypass' => "⏭️",
        'reapply' => "🔄", 'retry' => "🔄", 'refresh' => "🔄",
        'action'  => "⚡", 'req' => "⚡", 'prompt' => "⚡",
        ...
        // Process Flow
        'init'      => "⚙️", 'initialize' => "⚙️", 'setup' => "⚙️", 'boot' => "⚙️",
        'start'     => "🚀", 'starting' => "🚀", 'begin' => "🚀", 'launch' => "🚀", 'run' => "🚀",
        'waiting'   => "⏳", 'wait' => "⏳", 'pause' => "⏳", 'delay' => "⏳", 'sleep' => "⏳",
        'completed' => "🏁", 'complete' => "🏁", 'finish' => "🏁", 'finished' => "🏁", 'end' => "🏁",

        // Report & Dashboard Sections
        'dashboard' => "📊", 'summary' => "📊", 'metrics' => "📊", 'stats' => "📊",
        'report'    => "📋", 'ledger' => "📋", 'document' => "📋", 'file' => "📋",
        'wealth'    => "💰", 'finance' => "💰", 'money' => "💰", 'portfolio' => "💰",
        'users'     => "👥", 'accounts' => "👥", 'profiles' => "👥", 'clients' => "👥",
        'user'      => "👤", 'profile' => "👤", 'account' => "👤",
        'history'   => "📜", 'log' => "📜", 'archive' => "📜", 'records' => "📜",
        'vault'     => "🗄️", 'database' => "🗄️", 'db' => "🗄️", 'data' => "🗄️",
        'time'      => "🕒", 'date' => "🕒", 'clock' => "🕒", 'schedule' => "🕒",
        'security'  => "🔐", 'auth' => "🔐", 'protect' => "🔐",
        'settings'  => "🛠️", 'config' => "🛠️", 'tools' => "🛠️",
        'matrix'    => "🧩", 'grid' => "🧩", 'table' => "🧩",
        'company'   => "🏢", 'business' => "🏢", 'scrip' => "🏢", 'organization' => "🏢",
...
/**
    ...
    function encodeShortId($id) {
        // The Prime scatters sequential numbers widely
        $prime = 10007; 
        // The Salt Mask scrambles the bits (Change this to any random 7-digit number)
        $salt = 8483291; 
...
function get_market_signals($cacheDir) {
    $analyzedScrips = [];
    $globalStats = [
        'total_tracked' => 0, 'total_strong_sell' => 0,
        'avg_roi' => 0, 'highest_drawdown' => 0
    ];
...
function render_signal_cards($analyzedScrips) {
    ...
    foreach ($analyzedScrips as $scrip) {
        $verdict = "HOLD POSITION"; $colorTheme = "emerald"; $bgTheme = "bg-emerald-500"; $textTheme = "text-emerald-400"; $hexColor = "#10b981";
        
        if ($scrip['score'] >= 70) {
            $verdict = "STRONG SELL"; $colorTheme = "rose"; $bgTheme = "bg-rose-500"; $textTheme = "text-rose-500"; $hexColor = "#f43f5e";
        } elseif ($scrip['score'] >= 40) {
            $verdict = "REDUCE EXPOSURE"; $colorTheme = "amber"; $bgTheme = "bg-amber-500"; $textTheme = "text-amber-400"; $hexColor = "#f59e0b";
        }
